...
def vis_depth(depth, minmax=None, cmap=cv2.COLORMAP_JET, constant_max=120):
    ...
    depthmap_valid_count = (depthmap < 30).sum()
    constant_max = np.percentile(depthmap[depthmap<30], 99) if depthmap_valid_count > 10 else 30
    print('max depth for vis', constant_max)
    # constant_max = 1
    # constant_min = 0
