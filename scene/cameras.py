...
class Camera(nn.Module):
    def __init__(self, colmap_id, R, T, FoVx, FoVy, image, gt_alpha_mask,
        ...
        self.image_mask = torch.ones((1, self.image_height, self.image_width), device=self.data_device)

        if gt_alpha_mask is not None:
            self.image_mask = torch.tensor(gt_alpha_mask)[None].to(self.data_device)
            if len(self.image_mask.shape) == 4:
                self.image_mask = self.image_mask[0]
        self.gt_alpha_mask = self.image_mask

        self.original_image *= self.image_mask
        self.zfar = 100.0
        self.znear = 0.01
